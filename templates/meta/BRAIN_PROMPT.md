...
**Quality gate before adding any rule** (Source: arxiv 2602.11988): Verbose instruction files increase token cost by 20%+ while reducing success rates by 3%. Before adding a rule, ask: (1) Is this inferable from the codebase? If yes, skip it. (2) Is this preventing a real failure or addressing a measured risk? "Good practice" is not enough. (3) Can it be stated in ≤3 sentences? If not, it belongs in a skill file. Always update both the `.autoagent/` instance AND the `templates/` version.
