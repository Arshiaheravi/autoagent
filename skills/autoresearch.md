...
6. **Budget cap** — Max number of experiment cycles before stopping. Default: 20 cycles. Also stops on a wall-clock or spend ceiling. An uncapped loop on paid compute is banned (hard-loop-cap rule).
...
**Run autonomously — don't pause to ask if you should continue.** The user may be away. Run until any stop condition, then stop cleanly (do not run forever):
- The user manually stops you
- You hit the budget cap (default 20 cycles) or the wall-clock/spend ceiling
